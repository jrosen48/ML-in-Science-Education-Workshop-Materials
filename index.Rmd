...
- Trained a Support Vector Machine model (or algorithm) on 90% of the data
...
The _aim_ is different, the algorithms are methods of estimation are not (or, are differences in degree, rather thak in kind).
...
- Nothing too much, apart from computing power, time, and concerns of interpretability
...
- These probabilities are often changed (through the use of a threshhold - often 0.50) back into dichotomous or categorical labels
- Metrics for how well the model performed include:
  ...
  - *ROC AUC*: For different levels of the threshhold, what is the sensitivity and specificity?
